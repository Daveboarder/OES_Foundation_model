...
- Shared inputs: line database `external_data/Source/LIBS_data_vacuum.db` (`QuantParam`, `E_ion`, `PartF_var`; stages I and II only), concentration ranges from `Samples_Fe_matrix.xlsx` (one sample type per row; `Uncertainties` sheet gives ± ranges, default ±1 %; element columns absent from the DB are dropped → 38 elements), and the 17,428-point wavelength axis from `external_data/Data/VASKUT K8.json` (two CCD segments).
