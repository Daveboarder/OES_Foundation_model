...
# Resume: RESUME=Outputs/cf_pipeline_<ts> bash scripts/run_cf_pipeline.sh
# Every finished step records its artefact in <out>/state.env; on resume the
# steps whose variable is already set are skipped, so a crash in the middle
# costs only the step that failed.
set -uo pipefail
cd "$(dirname "$0")/.."
TS=$(date +%Y-%m-%d_%H-%M-%S)
OUT=${RESUME:-Outputs/cf_pipeline_$TS}
mkdir -p "$OUT"
STATE="$OUT/state.env"
touch "$STATE"
# shellcheck disable=SC1090
source "$STATE"                       # PRETRAIN / SEED_* / TOKENS_* from an earlier attempt
log() { echo "[$(date +%H:%M:%S)] $*"; }
run_dir_of() { grep -m1 "Run Directory:" "$1" | sed 's/.*Run Directory: *//' | tr -d '\r'; }
save() { echo "$1=$2" >> "$STATE"; }
have() { [ -n "${!1:-}" ]; }
skip() { log "skip STEP $1 (already have ${!2})"; }
...
if have TOKENS_SYN; then skip 1 TOKENS_SYN; else
...
TOKENS_SYN=$(grep -m1 "Tokens:" "$OUT/01_tokens_synthetic.log" | awk '{print $2}'); save TOKENS_SYN "$TOKENS_SYN"
LINE_DICT=$(grep -m1 "Dictionary:" "$OUT/01_tokens_synthetic.log" | awk '{print $2}'); save LINE_DICT "$LINE_DICT"
fi

if have TOKENS_MEAS; then skip 2 TOKENS_MEAS; else
...
TOKENS_MEAS=$(grep -m1 "Tokens:" "$OUT/02_tokens_measured.log" | awk '{print $2}'); save TOKENS_MEAS "$TOKENS_MEAS"
fi

if have PRETRAIN; then skip 3 PRETRAIN; PRE=$PRETRAIN; else
step "3 pretrain"
...
if have SEED_BINNED; then skip 4a SEED_BINNED; BIN=$SEED_BINNED; else
step "4a seed binned"
...
if have SEED_DETECTION; then skip 4b SEED_DETECTION; DET=$SEED_DETECTION; else
...
if have CF_LEARNED; then skip 5a CF_LEARNED; CFL=$CF_LEARNED; else
...
if have CF_PURE; then skip 5b CF_PURE; CFP=$CF_PURE; else
...
MTOK=${TOKENS_MEAS:-$(grep -m1 "Tokens:" "$OUT/02_tokens_measured.log" | awk '{print $2}')}
MCACHE=${MEAS_CACHE:-$(ls -t external_data/cache/measured_cache_*.h5 | head -1)}
LDICT=${LINE_DICT:-$(grep -m1 "Dictionary:" "$OUT/01_tokens_synthetic.log" | awk '{print $2}')}
